    ...
    <div className="text-on-surface h-full w-full">
      <main className="px-margin-mobile py-stack-lg max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          <h1 className="text-display font-display font-bold gradient-text">
            All Meetings
          </h1>
          <button
            ...
            {showFilterMenu && (
              <div className="absolute right-0 top-full mt-2 w-48 glass-panel shadow-premium rounded-2xl p-2 z-50 animate-in fade-in zoom-in-95 bg-surface/80 backdrop-blur-xl border border-outline-variant/30">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-xl transition-colors ${filterStatus === "all" ? "bg-primary/20 text-primary font-bold" : "hover:bg-white/40"}`}
                >
                  All Meetings
                </button>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-xl transition-colors ${filterStatus === "completed" ? "bg-status-success/20 text-status-success font-bold" : "hover:bg-white/40"}`}
                >
                  Completed
                </button>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 rounded-xl transition-colors ${filterStatus === "processing" ? "bg-primary/20 text-primary font-bold" : "hover:bg-white/40"}`}
                >
                  Processing
                </button>
              </div>
        ...
        <div className="flex flex-col gap-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center p-16 text-center glass-panel rounded-3xl animate-in fade-in">
              ...
              <p className="mt-3 text-body-lg text-on-surface-variant max-w-md mx-auto mb-8">
            ...
            <div className="flex flex-col items-center justify-center p-16 text-center glass-panel rounded-3xl animate-in fade-in">
              ...
              <p className="mt-3 text-body-lg text-on-surface-variant">
                We couldn&apos;t find any meetings matching your search.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4 gap-4">
                    <div className="flex flex-col gap-2 min-w-0">
                      <h3
                        className="text-headline-md font-bold text-on-surface truncate group-hover:text-primary transition-colors"
                        title={meeting.title}
                      >
                        {meeting.title}
                      </h3>
                    ...
                    <span
                      className={`flex-shrink-0 px-3 py-1.5 rounded-full text-label-md font-bold shadow-sm backdrop-blur-md ${meeting.status === "completed" ? "bg-status-success/20 text-status-success border border-status-success/30" : "bg-primary/20 text-primary border border-primary/30"}`}
                    >
                      {meeting.status}
                    </span>
                  </div>
                  {meeting.summary && (
                    <p className="text-body-lg text-on-surface-variant mb-4 line-clamp-2 bg-white/40 p-3 rounded-xl border border-white/50">
                      {meeting.summary}
                    </p>
                  )}
